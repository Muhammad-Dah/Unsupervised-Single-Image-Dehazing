...
class Models(object):
    ...
    def __init__(self, args):
        ...
        self.f_size = [
                          args.filter_size] * args.num_net_blocks  # vector the size of "rate" holding the filter size in each conv layer within the block
        ...
        self.rate = [2 ** i for i in
                     range(self.num_net_blocks)]  # vector of size of num dilated residual blocks holding each block's final layers' dilation rate
        self.pad_size = ((args.filter_size - 1) / 2) * (args.num_reg_conv * args.num_net_blocks + 1) + ((args.filter_size - 1) / 2) * np.sum(
            self.rate)

    def define_weights(self, f_size, in_size, out_size, init_std):

        weights = tf.Variable(tf.compat.v1.truncated_normal([f_size[0], f_size[1], in_size, out_size], stddev=init_std, seed=self.seed), "weights")
        return weights
    ...
    def my_conv(self, x, rate, f_size, in_size, out_size, stride, with_relu, phase, name):
        ...
        with tf.name_scope(name):
            ...
            conv = tf.compat.v1.layers.batch_normalization(conv + biases, training=phase)
            if with_relu:
    ...
    def dilated_skip_block(self, x, app, rate, f_size, in_size, out_size, stride, phase, name):
        ...
        with tf.name_scope(name):
            ...
            if self.padding_type != 'None':
                gap = int(((f_size - 1) / 2) * self.num_reg_conv + rate)
                out = tf.compat.v1.add(out, x[:, gap:-gap, gap:-gap, :])  # fusing]
            else:
    ...
    def build_model(self, x, phase):
        ...
        if self.model_type == 'conv_skip':
            ...
            out = x
            if (self.padding_type != 'None'):
                pad_size = int(self.pad_size)
                out = tf.compat.v1.pad(out , [[0, 0], [pad_size, pad_size], [pad_size, pad_size], [0, 0]], self.padding_type)
            out = self.my_conv(out, 0, self.f_size[0], self.num_classes + self.image_dim[2], self.num_filters[0], [1, 1, 1, 1], True, phase,
                               "first_conv")
            for i in range(len(self.rate)):
                out = self.dilated_skip_block(out, self.app, self.rate[i], self.f_size[i], self.num_filters[i - 1], self.num_filters[i], [1, 1, 1, 1],
                                              phase, "block_" + str(i + 1))
            out = self.my_conv(out, 0, 1, self.num_filters[-1], self.num_outputs, [1, 1, 1, 1], False, phase, "final_conv")

        if self.use_softmax == True:
            network_out = tf.nn.softmax(out, axis=-1, name='softmax')
        else:
            network_out = out
